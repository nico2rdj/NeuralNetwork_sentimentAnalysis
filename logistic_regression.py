...
# Data treatment
# Fill the lists of sentences and labels
file = open('wordListAmazon.txt','w')
for i in range(0,len(sentences)):
    for j in range(0,len(sentences[i])):
        if (sentences[i][j] != ' ' and sentences[i][j] != '.' and  sentences[i][j] != ',' and sentences[i][j] != '!' and sentences[i][j] != '-' and sentences[i][j] != '?' and sentences[i][j] != ')'):
            file.write(sentences[i][j])
        if (sentences[i][j] == ' ' or sentences[i][j] == '.' ):
            file.write('\n')
file.close()
...
w = np.zeros(1000) #((x_train.shape[1])) 
b = 0
